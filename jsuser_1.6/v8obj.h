...
namespace v8{
	...
	没有任何区别。*/
	/*@range_begin*/
#define ITOP(i) (void*)(UINT_PTR)i
#define PTOI(p)	(uint)(UINT_PTR)p
	inline bool is_bind_flag(uint flag){
		return 0!=flag&0x1;
	}
	...
	public:
		...
		static v8::Handle<Value> Create(const Arguments& args){
			...
			if(args.Length()>0&&args[0]->IsUint32()){
				...
				if((args.Length()<=1||args[1]->IsFalse())&&cobj!=0){
					setInternelPointer(self,1,ITOP(1));
				}else{
		...
		static Handle<Value> dispose(const Arguments& args){
			...
			uint flag = PTOI(getInternelPointer(self,1));
