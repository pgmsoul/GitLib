...
#define THREAD_FLAG_EXIT		2	//结束线程
	struct _StaticThreadParam : public _struct{
		...
		void Reset(){
			...
			Status = THREAD_STATUS_SUSPEND;
			Flag = THREAD_FLAG_SUSPEND;
		}
	};
	...
	public:
		...
		//成功返回一个Event指针，这个Event在指定的函数执行完后会被Signed, 此Event是一个自动阻塞事件，即使没有启动线程，它也会被阻塞，直到线程被启动而且返回才会signal。
	...
	*/
	...
	public:
		template<typename PROC> Event* StartStdTask(PROC cb,P1 p1,bool wait = 0){
			LocalCriticalSection lcs(_cs);
			if(_threadList.Count()>=_maxThreadCount) return 0;
			StaticThread2<R,P1>* pst = (StaticThread2<R,P1>*)_getThread();
			pst->SetCallStd(cb,p1);
			return pst->Start(wait);
		}
		template<typename OBJ,typename PROC> Event* StartTask(OBJ obj,PROC cb,P1 p1,bool wait = 0){
			LocalCriticalSection lcs(_cs);
			if(_threadList.Count()>=_maxThreadCount) return 0;
			StaticThread2<R,P1>* pst = (StaticThread2<R,P1>*)_getThread();
