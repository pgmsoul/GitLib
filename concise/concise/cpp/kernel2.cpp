...
namespace concise{
	...
	Event* _StaticThread::Start(bool wait){
		...
		if(!_Handle) return 0;
		if(_Handle->Handle==0){
			_Handle->Handle = ::CreateThread(0,0,_Handle->routine,_Handle,0,&_Handle->Id);
		}
		while(1){
			//Suspend to Running
	...
	StaticThread* _TaskPool::_getThread(){
	...
	//返回一个空闲的线程对象，如果所有激活线程都在运行，则返回一个新线程。
	_StaticThread* _TaskPool2::_getThread(){
		if(_threadList.Count()>=_maxThreadCount) return false;
		//LocalCriticalSection lcs(_cs);这个函数在使用之前已经锁定，这里无需锁定
		for(uint i=0;i<_threadList.Count();i++){
	...
	_TaskPool2::_TaskPool2(){
		_cleanPeroid = 100;
		_exitCleanThread = 0;
		_maxThreadCount = 1000;
	}
	void _TaskPool2::StartCleanThread(uint period){
		_cleanPeroid = period*10;
		_cleanThread.Call<void>(this,&_TaskPool2::_onClean);
	}
	void _TaskPool2::SetMaxThread(int maxCount){
		LocalCriticalSection lcs(_cs);
		_maxThreadCount = maxCount;
	}
	int _TaskPool2::GetTaskCount(){
		int n = 0;
		for(uint i=0;i<_threadList.Count();i++){
			if(_threadList[i].IsRunning()) 
				n++;
	...
	int _TaskPool2::GetThreadCount(){
		return _threadList.Count();
	}
	void _TaskPool2::Close(uint timeOut){
		_exitCleanThread = 1;
		_cleanThread.Wait();
		for(uint i=0;i<_threadList.Count();i++){
			_StaticThread& st = _threadList[i];
	...
	CriticalSection _prTaskCs;
	TaskPool* ProcessTaskPool(bool release){
		static TaskPool* taskPool = 0;
		LocalCriticalSection cs(_prTaskCs);
		if(release){
			if(taskPool) delete taskPool;
			return 0;
		}
		if(taskPool==0) taskPool = new TaskPool();
		return taskPool;
	}
