...
void __stdcall accept_request(int);
void bad_request(int);
void cat(int, cs::File& file);
...
/**********************************************************************/
void __stdcall accept_request(int client)
{
	char buf[1024];
	int numchars;
	char method[255];
	...
	i = 0; j = 0;
	while (buf[j]&&!ISspace(buf[j]) && (i < sizeof(method) - 1))
	{
		method[i] = buf[j];
		i++; j++;
	}
	method[i] = '\0';
	if (!cs::MbsEqualNoCase(method, "GET") && !cs::MbsEqualNoCase(method, "POST"))
	{
		unimplemented(client);
		return;
	}

	if (cs::MbsEqualNoCase(method, "POST"))
		cgi = 1;

	i = 0;
	while (buf[j]&&ISspace(buf[j]) && (j < sizeof(buf)))
		j++;
	while (buf[j]&&!ISspace(buf[j]) && (i < sizeof(url) - 1) && (j < sizeof(buf)))
	{
		url[i] = buf[j];
		i++; j++;
	}
	...
	if (cs::MbsEqualNoCase(method, "GET"))
	{
		query_string = url;
	...
	const char* wwwroot = "D:\\wamp\\www\\jsuser\\3%s";
	sprintf_s(path,512, wwwroot, url);//"htdocs%s"
...
/**********************************************************************/
void cat(int client, cs::File& file){
	char buf[1024];
	file.SetPointer(0);
	while (true){
				 ...
				 const char *method, const char *query_string)
{
	...
	buf[0] = 'A'; buf[1] = '\0';
	if (cs::MbsEqualNoCase(method, "GET") == 0)
		while ((numchars > 0) && strcmp("\n", buf))  // read & discard headers 
			numchars = get_line(client, buf, sizeof(buf));
	else    // POST 
	{
		...
		{
			buf[15] = '\0';
			if (cs::MbsEqualNoCase(buf, "Content-Length:") == 0)
	...
	if (pid == 0) // child: CGI script 
	{
		...
		close(cgi_output[1]);
		close(cgi_input[0]);
		if (cs::MbsEqualNoCase(method, "POST") == 0)
...
int wmain(void)
{
	...
	while (1)
	{
		...
		task.StartStdTask(accept_request,client_sock);
